...
前者は nil のスライスを宣言しますが、後者は nil ではなく長さ0のものになります。

JSON オブジェクトをエンコードする際など、限られた状況で nil ではなく長さ0のスライスのほうが好まれる状況があることに注目してください。
`nil` スライスは `null` に変換されますが、`[]string{}` は `[]` に変換されます。

インターフェースを設計するときは、nil のスライスと長さ0のスライスを区別しないようにしましょう。なぜなら微妙なプログラミングミスを引き起こすことがあるからです。

より詳しい議論は Francesc Campoy の [発表](https://www.youtube.com/watch?v=ynoY2xz-F8s)を参照してください
...
同じことが [initialisms]( https://qiita.com/knsh14/items/8b73b31822c109d4c497#initialisms ) でも説明されています。
...
# Contexts
`context.Context` 型はセキュリティの為の証明情報、デバッグトレースのための情報、デッドライン(どう訳そう?)、そして API やプロセス間のキャンセル処理のためのシグナルを持っています。
Go のプログラムは入ってきた RPC や HTTP リクエストから、でていくリクエストに向けて関数呼び出しの流れに沿って明示的に Context を渡します。
...
A function that is never request-specific may use context.Background(), but err on the side of passing a Context even if you think you don't need to.
...
# Goroutine Lifetimes
goroutine を生成する時、いつ終了されるか明確にして置かなければ行けません。
goroutine は channel の送受信をブロックによってメモリリークを起こす場合があります。
ガベージコレクタはブロックされている channel に到達できなくても、goroutine を停止させません。

もしリークしていなかった場合でも、必要にならなくなった物を残しておくのは、微妙で調査しにくい問題を起こすことになりかねません。
クローズした channel に何をかを送ると panic を起こして終了してくれます。
